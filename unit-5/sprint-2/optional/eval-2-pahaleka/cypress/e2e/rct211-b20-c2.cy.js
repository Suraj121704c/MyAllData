...
data.forEach(({ submission_link: url, id }) => {
  describe("Evaluation RCT-211-B120-E2", function () {
    ...
    it(`Check if isLoading is being updated while visiting coffee page`, () => {
      ...
      cy.window().then(getCurrentState).its("isLoading").should("eq", false);

      cy.visit(url);

      cy.get(".navbar").children().eq(0).click().then(() => {
          cy.window().then(getCurrentState).its("isLoading").should("eq", true);
        });
    ...
    it(`All coffee should be displayed on the DOM`, () => {
      cy.url().should("eq", url);
      cy.get(".navbar").children().eq(0).click();

      cy.get(".coffee_data").children().should("have.length", 20);
      cy.then(() => (acc_score += 1));
    });
    ...
    it(`All restaurant should be displayed on the DOM`, () => {
      cy.url().should("eq", url);
      cy.get(".navbar").children().eq(2).click();

      cy.get(".restaurant_data").children().should("have.length", 5);
      cy.then(() => (acc_score += 1));
    });
    ...
    it(`All employee should be displayed on the DOM`, () => {
      cy.url().should("eq", url);
      cy.get(".navbar").children().eq(1).click();

      cy.get(".employee_data").children().should("have.length", 100);
      cy.then(() => (acc_score += 1));
    });
